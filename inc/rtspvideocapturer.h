...
class RTSPAudioSource : public webrtc::LocalAudioSource, public rtc::Thread, public RTSPConnection::Callback {
	public:
		static rtc::scoped_refptr<RTSPAudioSource> Create(const std::string & uri) {
			rtc::scoped_refptr<RTSPAudioSource> source(new rtc::RefCountedObject<RTSPAudioSource>(uri));
			return source;
		}
...
//						std::cout << "pop size:" << m_buffer.size() << std::endl;
	...
	protected:
		RTSPAudioSource(const std::string & uri) : m_connection(m_env, this, uri.c_str()), m_factory(webrtc::CreateBuiltinAudioDecoderFactory()), m_sink(NULL), m_freq(8000), m_channel(1) { rtc::Thread::Start(); }
