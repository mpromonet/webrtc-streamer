...
RUN apt-get update && apt-get install -y --no-install-recommends g++ autoconf automake libtool xz-utils libpulse-dev libasound2-dev libgtk-3-dev libxtst-dev cmake p7zip-full \
...
RUN apt-get update && apt-get install -y --no-install-recommends libasound2 libgtk-3-0 libxtst6 libssl1.0 \
	&& apt-get clean && rm -rf /var/lib/apt/lists/ \
	&& ./webrtc-streamer -V
